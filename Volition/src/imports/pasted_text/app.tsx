...
export default function App() {
  ...
  const [isMobileHeaderExpanded, setIsMobileHeaderExpanded] = useState(false); // NEW STATE
  ...
  const handleNodeInspect = (node: any) => {
    ...
    setInspectedNode({
      ...
      // SAFETY FALLBACK: Ensure MonologueBanner doesn't crash if it expects 'title' instead of 'name'
    ...
    // FIX: Only trigger the Mobile Banner if we are actually on a phone screen!
  ...
  const handleCloseInspector = () => {
    ...
    setIsMobileBannerOpen(false); // CRITICAL: Release the mobile UI lock!
  ...
  // Add this function right below your handleCloseInspector function:
  ...
  const executeSearch = async () => {
    ...
    try {
      ...
      setIsMobileBannerOpen(true); // Pop open the UI on mobile so they can see results
  ...
  return (
    ...
    {/* Changed to w-full to prevent horizontal scroll breaking, unified the background */}
    ...
    <div className="relative w-full h-screen overflow-hidden bg-black font-['Share_Tech_Mono'] text-white selection:bg-white/30 selection:text-white">
      ...
      <div className={`absolute inset-0 pointer-events-none z-20 transition-all duration-700 ${isZenMode ? 'opacity-0 [&_*]:!pointer-events-none' : 'opacity-100'}`}>
        ...
        {/* Top Navigation - Fixed: Removed the right-[30vw] that was squishing your UI */}
